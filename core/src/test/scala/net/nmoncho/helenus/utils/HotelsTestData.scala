...
object HotelsTestData:
    import net.nmoncho.helenus.models.*

    private val rnd = new Random(0)

    def insertTestData()(implicit session: CqlSession): Unit =
        val insertHotelByPOI =
            "INSERT INTO hotels_by_poi(poi_name, hotel_id, name, phone, address) VALUES (?, ?, ?, ?, ?)".toCQL
                .prepare[String, String, String, String, Address]
        val insertPoiByHotel =
            "INSERT INTO pois_by_hotel(poi_name, hotel_id, description) VALUES (?, ?, ?)".toCQL
        ...
        val insertAvailableRooms =
            "INSERT INTO available_rooms_by_hotel_date(hotel_id, date, room_number, is_available) VALUES (?, ?, ?, ?)".toCQL
        ...
        val insertAmenities =
            "INSERT INTO amenities_by_room(hotel_id, room_number, amenity_name, description) VALUES (?, ?, ?, ?)".toCQL
        ...
        do insertAmenities.execute(hotel.id, room.toShort, amenity.name, amenity.description)
        end for
    end insertTestData
    ...
    end PointOfInterests

    object Hotels:

        import PointOfInterests.*
