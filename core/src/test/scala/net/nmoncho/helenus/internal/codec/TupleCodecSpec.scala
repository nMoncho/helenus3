...
class TupleCodecSpec extends AnyWordSpec with Matchers with CodecSpecBase[(String, String)]:
    ...
    override protected val codec: Codec[(String, String)] = Codec[(String, String)]
    ...
    "Tuple2Codec" should {
        ...
        "accept generic type" in {
            ...
            codec.accepts(intTupleCodec.getJavaType) shouldBe false // FIXME
        }
...
class OnParTupleCodecSpec
    ...
    override protected val codec: Codec[(String, Int)] = Codec[(String, Int)]

    override def javaCodec: TypeCodec[TupleValue] = TypeCodecs.tupleOf(tupleType)

    override def toJava(t: (String, Int)): TupleValue = if t == null then null
    else
        tupleType
            .newValue()
            .setString(0, t._1)
